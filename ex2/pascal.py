...
root_dir = os.getcwd()
root_dir += '/pascal/train/VOCdevkit/VOC2007/'
...
"""
    name is like 17
    output is like 000017
"""
def convert_im_name(name):
    tmp_name = str(name)
    return '0' * (6 - len(tmp_name)) + tmp_name
...
def annotation_file_from_img(img_name):
    ...
    """
    return os.path.join(ann_dir, convert_im_name(img_name)) + '.xml'
...
def load_data_multilabel(data_type=None):
    ...
    return df
"""
crop an image on its first two dimensions
"""
def crop_random_patch(img, height, width):
    x0 = np.random.randint(0, img.shape[0] - width)
    y0 = np.random.randint(0, img.shape[1] - height)
    return img[x0:x0+width, y0: y0+ height]


def prepare_data_facedet(width = 12, height = 12, crops_per_image = 1):
    data = []
    data_type = 'train'
    categories = list_image_sets()
    categories.remove('person') # interested to find non faces
    categories.remove('aeroplane') # buggy
    for category in categories:
        imgs_url = get_image_url_list(category, data_type)
        for img_url in imgs_url:
            #load image
            img = scipy.ndimage.imread(img_url, flatten=False)
            #get crop
            for i in range(crops_per_image):
                crop = crop_random_patch(img, height, width)
                #add it to data returned
                data.append(crop)
    return np.asarray(data).reshape(len(data), 3, width, height).astype(np.float64) / 255 # 3 for rgb
